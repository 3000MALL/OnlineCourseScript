...
getConfigFileInfo() {
    ...
    case "$PROTOCOL" in
        ...
        "vless")
            ...
            local DM=${DOMAIN%%.*}
            REMARK="$(cat /etc/timezone)_$CURRENT_DATE$DM"
            ENCRYPTION="none"
            ;;
        "vmess")
            if [[ "$NETWORK" == "ws" ]]; then
...
showInfo() {
    ...
    [[ -n "$PASSWORD" ]] && echo -e "   ${BLUE}密码(password): ${PLAIN}${RED}${PASSWORD}${PLAIN}"
    ...
    [[ -n "$DOMAIN" ]] && echo -e "   ${BLUE}伪装域名/主机名(host)/SNI/peer名称: ${PLAIN}${RED}${DOMAIN}${PLAIN}"
